...
@section('content')
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Edit Status Services</h4>
        </div>
        <div class="modal-body">
          <p>Apakah anda yakin untuk mengubah status services ini?</p>
        </div>
  ...
  <div class="modal fade" id="modaldelete" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Hapus Services</h4>
        </div>
        <div class="modal-body">
          <p>Apakah anda yakin untuk menghapus services ini?</p>
        </div>
        <div class="modal-footer">
          <button type="reset" class="btn btn-default pull-left btn-flat" data-dismiss="modal">Tidak</button>
  ...
  <div class="modal fade" id="modaledit" role="dialog">
    <div class="modal-dialog">
      <form class="form-horizontal" action="{{route('services.edit')}}" method="post" enctype="multipart/form-data">
        {{csrf_field()}}
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title">Edit Konten Services</h4>
          </div>
          <div class="modal-body">
            <div class="col-md-14">
              <label class="control-label">Nama Services</label>
              <input type="text" name="nama_services" class="form-control" id="edit_nama_services">
            </div>
            <div class="col-md-14">
            ...
            <div class="col-md-14">
              ...
              <select class="form-control" name="flag_services">
      ...
      <form class="form-horizontal" action="{{route('services.store')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="box box-success">
          <div class="box-header with-border">
            <h3 class="box-title">Formulir Tambah Services Baru</h3>
          </div>
          <div class="box-body">
            <div class="col-md-14">
              <label class="control-label">Nama Services</label>
              <input type="text" name="nama_services" class="form-control">
            </div>
            <div class="col-md-14">
              <label class="control-label">Keterangan</label>
    ...
    <div class="col-md-8">
      <div class="box box-success">
        <div class="box-header">
          <h3 class="box-title">Seluruh Data Services</h3>
        </div><!-- /.box-header -->
        <div class="box-body">
            ...
            <tbody>
              ...
              @foreach($getservices as $key)
                <tr>
                  ...
                  <td>{{$key->keterangan_service}}</td>
                  <td>
                    @if($key->url_service!="")
                      <?php $gambar = explode(".", $key->url_service) ?>
                      <img src="{{url('images/')}}/{{$gambar[0]}}_200x122.{{$gambar[1]}}"></td>
                    @else
                      <img src="{{url('images/')}}/no_image.jpg"></td>
                    @endif
                  <td>
                    @if($key->flag_service=="1")
  ...
  <script>
    $(function () {
      ...
      $("#tabelinfo").on("click", "a.flagpublish", function(){
        ...
        $('#setflagpublish').attr('href', '{{url('admin/publish-services/')}}/'+a);
      });

      $("#tabelinfo").on("click", "a.hapus", function(){
        var a = $(this).data('value');
        $('#sethapus').attr('href', '{{url('admin/delete-services/')}}/'+a);
      });

      $("#tabelinfo").on("click", "a.edit", function(){
        var a = $(this).data('value');
        $.ajax({
          url: "{{url('/')}}/admin/bind-services/"+a,
          dataType: 'json',
          success: function(data){
            var id = data.id;
            var nama_services = data.nama_service;
            var keterangan_services = data.keterangan_service;
            var flag_services = data.flag_service;

            $('#id').attr('value', id);
            $('#edit_nama_services').val(nama_services);
            $('#edit_keterangan_services').val(keterangan_services);
            if(flag_services=="1") {
